...
![1](1.png)当用户点击File选项时弹出Open选项，点击即可打开一个文件栏，读取固定格式的txt文件，可以完成回放的功能。

![2](2.png)点击Control按钮会弹出选项start，点击即可开始运行战斗。空格键有相同的效果。战斗前可以在controller中的formation选项中调整阵型。会弹出对话框以供选择。

![3](3.png)

当正义的葫芦娃一方或者邪恶的怪兽一方全部死亡时，战斗结束。如果葫芦娃失败，则播放失败图标，否则播放成功图标。

![4](4.png)当战斗还未开始或者已经结束后，再次按空格键或L键可以选择战斗或者回放。
...
## 设计原则 ##
### SRP单一职责原则 ###
- Creature类及其派生类void run（）中只进行生物体的移动和战斗判定，战斗的记录由BattleGround类负责。
- 游戏内部运行与图形化展示分离开。
### OCP开发封闭原则 ###
...
### CARP 合成/聚合复用原则 ###
- Model类中存储了GOOD，BAD, BattleGround等。
## 心得体悟
- JAVA编程对设计的需求很高。
- 尽量采用面向对象的设计方法来完成程序设计。
## 使用要点注意
