...
class Conductor extends Creature{       //指挥官类，爷爷和蛇精的父类
    ...
    public void Conduct(Creature c, int nx, int ny){};//虚方法，指挥自己的一方排兵布阵，11.10再次尝试在子类中覆写成功 :)，在子类中加入了确保指挥的类型为自己可以指挥的本方角色
}
...
final class Grandpa extends Conductor{     //爷爷类
    ...
    */
    ...
    @Override   //11.10重新尝试覆写，祝我好运 :)
    public void Conduct(Creature c, int nx, int ny) //参数和返回值保证相同
    {
        if(c.getClass() == Calabash.class)
        {
            c.Goto(nx, ny);
        }
        else System.out.println("爷爷不能指挥非葫芦娃！");
    }
}
...
final class Snake extends Conductor{     //蛇精类
    ...
    */
    ...
    public void Conduct(Creature c, int nx, int ny) //参数和返回值保证相同
    {
        ...
        else System.out.println("蛇精不能指挥非小妖精！");
    }
}
